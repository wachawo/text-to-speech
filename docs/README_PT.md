...
| GET | `/api/voices?engine=&language=` | Vozes de um motor; para `coquitts`, as amostras com tamanho, taxa e duração. |
...
| POST, GET | `/api/tts` | Sintetiza `text` com `engine`, `language`, `voice`; `stream=true` transmite os trechos conforme ficam prontos. |
...
`language` é um código de duas letras (`en`, `ru`) ou uma tag com região ou escrita (`zh-cn`, `pt_BR`, `en-gb`, `es-419`), convertida para minúsculas e escrita com `-`. Cada motor converte a tag no que tem: o gtts recebe a própria grafia (`zh-CN`; ele não tem francês canadense nem português europeu, então `fr-ca` e `pt-pt` viram `fr` e `pt`), o kokorotts fala `en-gb` com o fonemizador britânico, o xtts recebe `zh-cn` para chinês e os demais motores usam a parte do idioma (`pt-br` vira `pt`). Com `stream=true`, o campo `language` da requisição aceita apenas códigos de duas letras, como antes; uma requisição sem `language` usa `TTS_LANGUAGE` como está definido, mesmo que seja uma tag. Um idioma que o motor não conhece é falado no idioma padrão dele, normalmente inglês (o gtts falha em vez disso); com `TTS_LANGUAGE_STRICT=true` é um 400 que lista os idiomas do motor. A verificação estrita vale para `/api/tts` sem `stream`, `/api/history` e `/v1/audio/speech`, e para todos os motores que listam seus idiomas: todos exceto o pyttsx3 e o coquitts com um modelo multilíngue que não seja o xtts.
