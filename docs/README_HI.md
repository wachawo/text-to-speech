...
- `language` एक एक्सटेंशन है: दो-अक्षर का कोड, डिफ़ॉल्ट `TTS_LANGUAGE`।
...
जब `TTS_TOKENS` सेट हो, तो `/api/health` को छोड़कर हर रूट को `Authorization: Bearer <token>` की ज़रूरत होती है। `/api/` के अंतर्गत त्रुटियाँ `{"error": "...", "request_id": "..."}` होती हैं; `/v1/` के अंतर्गत त्रुटियाँ OpenAI के प्रारूप का उपयोग करती हैं।
