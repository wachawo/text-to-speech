...
| `TTS_MODEL_CACHE_SIZE` | `2` | coquitts と kokorotts が同時に読み込んでおくモデルの数。さらに別のモデルを求めるリクエストは、最初に読み込んだモデルを解放する。 |
