...
```bash
# Что предлагает один движок: модели, языки и голоса, без загрузки модели
curl localhost:5000/api/engines/pipertts \
  -H "Authorization: Bearer $TTS_TOKEN"

# Синтезировать одной из перечисленных там моделей
curl -X POST localhost:5000/api/tts \
  -H "Authorization: Bearer $TTS_TOKEN" \
  -H "Content-Type: application/json" \
...
- `model` - имя движка либо `tts-1` / `tts-1-hd` / `gpt-4o-mini-tts` для движка по умолчанию. Выбрать модель внутри движка на `/v1` нельзя: движок использует свою модель по умолчанию.
...
- `language` - расширение: двухбуквенный код или тег вроде `zh-cn`, по умолчанию `TTS_LANGUAGE`.
...
Каждый маршрут, кроме `/api/health`, требует `Authorization: Bearer <token>`, если задан `TTS_TOKENS`. Ошибки под `/api/` имеют вид `{"error": "...", "request_id": "..."}`; ответ 400 дополнительно содержит `message` с причиной, а если запрос не прошёл проверку, в нём названы ошибочные поля (`language: ...`). Ошибки под `/v1/` используют формат OpenAI.
...
| GET | `/api/engines/<engine>` | Модели, языки, список голосов и формат вывода одного движка; модели при этом не загружаются. |
| GET | `/api/models` | Установленные и отсутствующие модели по движкам, та же таблица, что печатает `ttsgen --list`. |
| GET | `/api/voices?engine=&language=&model=` | Голоса движка; для `coquitts` - образцы с размером, частотой и длительностью. С `model` - голоса этой модели. |
...
| POST, GET | `/api/tts` | Синтезировать `text` с `engine`, `language`, `voice`, `model`; `stream=true` отдаёт фрагменты по мере готовности. |
...
`language` - двухбуквенный код (`en`, `ru`) или тег с регионом или письменностью (`zh-cn`, `pt_BR`, `en-gb`, `es-419`); тег приводится к нижнему регистру и пишется через `-`. Каждый движок сводит тег к тому, что у него есть: gtts получает собственное написание (`zh-CN`; канадского французского и европейского португальского у него нет, поэтому `fr-ca` и `pt-pt` становятся `fr` и `pt`), kokorotts произносит `en-gb` с британским фонемизатором, xtts получает `zh-cn` для китайского, а остальные движки берут часть с языком (`pt-br` становится `pt`). Язык, которого движок не знает, озвучивается языком движка по умолчанию, обычно английским (gtts вместо этого возвращает ошибку); при `TTS_LANGUAGE_STRICT=true` такой запрос получает 400 со списком языков движка. Строгая проверка действует на `/api/tts` без `stream`, `/api/history` и `/v1/audio/speech` и на все движки, которые перечисляют свои языки: на все, кроме pyttsx3, pipertts без установленных голосов и coquitts с многоязычной моделью, отличной от xtts, или с моделью, у которой код языка из трёх букв (`ewe`).

`model` выбирает модель внутри движка на `/api/tts` и `/api/history`: голос Piper (`en_GB-alan-low`), файл модели Kokoro (`kokoro-v1.0.int8.onnx`), модель Silero (`v3_1_ru`) или имя модели Coqui (`tts_models/de/thorsten/vits`). `GET /api/engines/<engine>` перечисляет их в `models`, у каждой указаны `languages`, `installed` и `default_for` (языки, для которых она используется, если запрос не называет модель), а рядом `languages` движка, `output_format`, `max_text_length` и признак того, есть ли у движка список голосов; модели при этом не загружаются, а на неизвестный движок приходит 404. Без `model` движок выбирает модель как раньше; id, которого нет в списке движка, получает 400 с перечислением доступных, как и `model` вместе со `stream=true`, потому что поток всегда использует модель движка по умолчанию. У gtts, pyttsx3 и barktts моделей нет. Без модели pipertts выбирает среди установленных голосов: тег с регионом (`en-gb`) берёт голос этого региона, а язык вне встроенной таблицы берёт установленный голос этого языка вместо английского; список языков pipertts - это языки установленных голосов.
...
| `TTS_LANGUAGE_STRICT` | `false` | `true` отвечает 400 на язык, которого нет в списке движка, вместо отката движка на язык по умолчанию. |
| `TTS_MODEL_CACHE_SIZE` | `2` | Сколько моделей coquitts и kokorotts держат загруженными одновременно; запрос ещё одной выгружает модель, загруженную раньше всех. |
