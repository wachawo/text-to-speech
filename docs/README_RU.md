...
`language` - двухбуквенный код (`en`, `ru`) или тег с регионом или письменностью (`zh-cn`, `pt_BR`, `en-gb`, `es-419`); тег приводится к нижнему регистру и пишется через `-`. Каждый движок сводит тег к тому, что у него есть: gtts получает собственное написание (`zh-CN`), kokorotts произносит `en-gb` с британским фонемизатором, xtts получает `zh-cn` для китайского, а остальные движки берут часть с языком (`pt-br` становится `pt`). `stream=true`, как и раньше, принимает только двухбуквенные коды. Язык, которого движок не знает, озвучивается языком движка по умолчанию, обычно английским (gtts вместо этого возвращает ошибку); при `TTS_LANGUAGE_STRICT=true` такой запрос получает 400 со списком языков движка. Строгая проверка действует на `/api/tts` без `stream`, `/api/history` и `/v1/audio/speech` и на все движки, кроме pyttsx3, который не перечисляет свои языки.
